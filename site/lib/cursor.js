...
// packet DRAFT: a starting point the user refines by hand and then passes to
// `pack()`. Like every adapter it is deliberately NOT a validated, final packet —
// a session can't know the working state, the real next steps, or which files
// matter, so this only seeds the sections it can reasonably infer:
//
...
//
// Cursor's export shape isn't fixed, so this is pragmatic and tolerant: it reads
// a whole-document JSON chat (`[…]` or `{ messages: […] }`), a `.jsonl` stream of
// message objects, OR a plain `User:` / `Assistant:` prose transcript — whichever
// the input turns out to be. Anything that won't parse is skipped, never thrown.
// No I/O, no clock — the caller supplies `created` when it finally `pack()`s.
...
// Lift {role, text} messages out of a whole-document JSON chat or a `.jsonl`
...
function structuredMessages(raw) {
  ...
  for (const line of s.split("\n")) {
    ...
    try {
      ...
      // not JSON — handled by the prose fallback in fromCursor
    }
  }
  return out;
}
...
function proseMessages(raw) {
  ...
  for (const line of String(raw ?? "").split("\n")) {
    const m = /^\s*(user|human|assistant|ai|model)\s*:\s*(.*)$/i.exec(line);
    if (m) {
      flush();
...
// fromCursor(rawSession, opts) → a partial packet draft.
//
//   rawSession      — a Cursor chat export (JSON / `.jsonl`) or `User:`/`Assistant:`
//                     prose. A marker-less blob is read as a rules-style scratchpad.
...
export function fromCursor(rawSession, opts = {}) {
  ...
  // Marker-less scratchpad: no user/assistant turns at all. Read it pragmatically
  // — the first prose line is the ask, every bullet is a progress note.
